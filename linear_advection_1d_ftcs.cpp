...
int main()
{
    ...
    double C = 0.5;//dt<=C*dx/u C is the CFL numbers
    ...
    double t_tot = 0.1 * t_zhouqi; //one zhou qi
    double t = 0.0;//time
    ...
    while(t<t_tot)
    {
        ...
        { //eq 4.2 -4.5 in Zingale book page 61
            ...
            //cell_val_update[i] = cell_val[i]- C*(cell_val[i]-cell_val[i-1]);//upwinding
            cell_val_update[i] = cell_val[i] - 0.5*C*(cell_val[i+1]-cell_val[i-1]);//ftcs
        }
        for(i=nghost;i<=nghost+ncells-1;i++)
        {
    ...
    ofstream myfile("ftcs1dlinadvc05.dat");
    myfile.precision(10);
